% Rebuttal-ready one-page vision summary table
% Fine-grained datasets + heterogeneous het5 stream

\begin{table*}[ht]
\centering
\caption{%
  One-page summary of the supplementary vision results used in the rebuttal.
  Values are the raw continual-accuracy summaries reported for the five
  fine-grained datasets and the heterogeneous cross-dataset stream
  \texttt{het5} ($\text{Aircraft} \rightarrow \text{Cars196} \rightarrow
  \text{CUB200} \rightarrow \text{Flowers} \rightarrow \text{OxfordPet}$).
  All runs use a frozen ViT-B/16 backbone pretrained on ImageNet-21K.
  ``$-$'' indicates that the corresponding \texttt{het5} run is not yet available.
}
\label{tab:rebuttal_vision_onepage}
\setlength{\tabcolsep}{4.0pt}
\renewcommand{\arraystretch}{0.92}
\scriptsize
\begin{tabular}{@{}l l r r r r r r@{}}
\toprule
\textbf{Method} & \textbf{Opt.} & \textbf{Aircraft} & \textbf{Cars196} & \textbf{CUB200} & \textbf{Flowers} & \textbf{OxfordPet} & \textbf{het5} \\
\midrule
