...
\title{Flat-Minima Optimization and Fisher Information Guided Alignment for Increamental LoRA in Continual Learning}
...
\begin{abstract}
Continual learning with large-scale models requires balancing efficient adaptation, knowledge retention, and robust generalization across tasks.
\LYY{First sentence: In the continual learning of large models, the incremental LoRA method often falls into the trap of "sharp minima", which not only impairs the model's generalization ability to new tasks, but also exacerbates the catastrophic forgetting of old tasks.} Motivated by these challenges, we first establish a novel PAC-Bayesian generalization bound for Multi-LoRA in continual learning, which highlights the joint roles of flat loss landscape, and independence of different LoRA   parameters and transfer of knowledge between different tasks. Building on this theoretical insight, we propose Fisher Information guided Flat-Minima Orthogonal Incremental LoRA (FFOI), a novel method that unifies sharpness-aware empirical risk minimization with Fisher information-based flatness regularization and orthogonality constraints among LoRA adapters.L
\end{abstract}
...
\subsection{PAC-Bayes Generalization Bound with Flatness for Incremental LoRA}
...
\begin{figure}[htp]
  \centering
  \includegraphics[width=1\linewidth]{show_algorithm.pdf}
