...
from efm_trust import EFMTrustRegionHelper
    ...
    def _train(self, train_loader, test_loader, mode='train'):
        ...
        for epoch in range(1, epochs + 1):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                loss_on_new = not (self.args.get('cls_fixed', False) and self.args['cls_fixed'])
                if not loss_on_new:
                    loss = F.cross_entropy(logits, cur_targets)
                else:
                    loss = F.cross_entropy(logits[:, self._known_classes:], cur_targets)

                # ---- 叠加 EFM trust-region（与 CE 的切片保持一致）----
