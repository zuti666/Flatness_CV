...
class BaseLearner(object):
    ...
    def compute_all_seen_class_means(self, data_manager) -> None:
        """Compute per-class normalized feature means for all seen classes (for NME eval)."""
        ...
        for c in range(nb_classes):
            try:
                ...
                logging.exception("[BaseLearner][NME] Failed to compute mean for class %d: %s", c, exc)
    ...
    def _build_optimizer(self, params, stage):
        ...
        if self._optimizer_type == "ngd" or self._optimizer_type == "naturalgradient":
            # Simple diagonal natural gradient (EMA Fisher) preconditioner
            from optimer_PerturabtionType.natural_gradient import NaturalGradient
            return NaturalGradient(
                params,
                lr=lr,
                damping=float(self.args.get("ngd_damping", 1e-3)),
                ema_decay=float(self.args.get("ngd_ema_decay", 0.95)),
                eps=float(self.args.get("ngd_eps", 1e-8)),
