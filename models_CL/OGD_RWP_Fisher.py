"""OGD + RWP with Fisher-aware Gaussian perturbation.

Implementation summary:
- Drift control: OGD projects gradients to the orthogonal complement of stored
  old-task gradient directions.
- Diffusion control: RWP perturbation is sampled before forward/backward and
  shaped by old-task Fisher geometry.
...
class Learner(BaseLearner):
    ...
    def incremental_train(self, data_manager):
        ...
        try:
            ...
            logging.exception("[OGD_RWP_FISHER] Failed to compute class means: %s", exc)
    ...
    def _align_directions_to_dim(self, target_dim: int) -> None:
        ...
        logger.info("[OGD_RWP_FISHER] Aligned %d directions to dim=%d", len(self._directions), target_dim)
    ...
    def _compute_fisher(self, loader, model: nn.Module) -> Dict[str, torch.Tensor]:
        ...
        logger.info("[OGD_RWP_FISHER] Fisher estimated over %d batches", total)
        return self._fisher
    ...
    def _rebuild_fisher_projection(self) -> None:
        ...
        logger.info(
            "[OGD_RWP_FISHER] Fisher projection built: topk=%d, covered_dims=%d, params=%d",
            k,
            covered,
    ...
    def _update_ogd_directions(self, loader) -> None:
        ...
        logger.info("[OGD_RWP_FISHER] Stored global dirs %d", len(self._directions))
