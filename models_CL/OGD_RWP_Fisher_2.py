"""OGD + RWP with Fisher-inverse-root diffusion and OGD-orthogonal noise.

Target perturbation:
    delta = sigma * P_perp * F^{-1/2} * z,  z ~ N(0, I)

Design:
- RWP two-step update is preserved: perturb -> grad at perturbed point ->
  OGD project grad -> restore -> optimizer.step.
- Fisher is diagonal (EWCon-style running estimate).
- Noise orientation and drift control share the same global OGD projector.
"""
...
class Learner(BaseLearner):
    def __init__(self, args):
        ...
        # Optional global L2 clip on projected preconditioned noise.
    ...
    def incremental_train(self, data_manager):
        ...
        try:
            ...
            logging.exception("[OGD_RWP_FISHER_2] Failed to compute class means: %s", exc)
        ...
        for epoch in prog_bar:
            ...
            for batch in train_loader:
                ...
                try:
                    ...
                    # OGD drift projection on perturbed-point gradient.
    ...
    def _sample_and_apply_fisher_rwp(self, model: nn.Module, optimizer) -> List[Tuple[torch.nn.Parameter, torch.Tensor]]:
        ...
        for name, p in self._iter_trainable(model):
            ...
            fisher = None if self._fisher is None else self._fisher.get(name)
            if fisher is not None:
                if fisher.shape != p.shape:
                    fisher = self._match_tensor(fisher, p)
                fisher = fisher.to(device=p.device, dtype=p.dtype)
                inv_sqrt = torch.rsqrt(torch.clamp(fisher, min=self._fisher_eps))
                z = z * inv_sqrt
            params.append(p)
            blocks.append(z.reshape(-1))

        if not blocks:
            return []

        flat_noise = torch.cat(blocks, dim=0)
        if self._directions:
        ...
        if offset != flat_noise.numel():
            logger.warning(
                "[OGD_RWP_FISHER_2] Noise split mismatch: used=%d total=%d",
                offset,
                flat_noise.numel(),
            )
    ...
    def _align_directions_to_dim(self, target_dim: int) -> None:
        ...
        logger.info("[OGD_RWP_FISHER_2] Aligned %d directions to dim=%d", len(self._directions), target_dim)
    ...
    def _compute_fisher(self, loader, model: nn.Module) -> Dict[str, torch.Tensor]:
        ...
        logger.info("[OGD_RWP_FISHER_2] Fisher estimated over %d batches", total)
        return self._fisher
    ...
    def _update_ogd_directions(self, loader) -> None:
        ...
        logger.info("[OGD_RWP_FISHER_2] Stored global dirs %d", len(self._directions))
