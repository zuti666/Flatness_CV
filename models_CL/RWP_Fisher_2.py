"""RWP with Fisher-inverse-root diffusion (no OGD constraints).

Target perturbation:
    delta = sigma * F^{-1/2} * z,  z ~ N(0, I)
...
class Learner(BaseLearner):
    ...
    def incremental_train(self, data_manager):
        ...
        try:
            ...
            logging.exception("[RWP_FISHER_2] Failed to compute class means: %s", exc)
    ...
    def _sample_and_apply_fisher_rwp(self, model: nn.Module, optimizer) -> List[Tuple[torch.nn.Parameter, torch.Tensor]]:
        ...
        if offset != flat_noise.numel():
            logger.warning(
                "[RWP_FISHER_2] Noise split mismatch: used=%d total=%d",
                offset,
                flat_noise.numel(),
            )
    ...
    def _align_directions_to_dim(self, target_dim: int) -> None:
        ...
        logger.info("[RWP_FISHER_2] Aligned %d directions to dim=%d", len(self._directions), target_dim)
    ...
    def _compute_fisher(self, loader, model: nn.Module) -> Dict[str, torch.Tensor]:
        ...
        logger.info("[RWP_FISHER_2] Fisher estimated over %d batches", total)
        return self._fisher
    ...
    def _update_ogd_directions(self, loader) -> None:
        ...
        logger.info("[RWP_FISHER_2] Stored global dirs %d", len(self._directions))
