...
def accuracy(y_pred, y_true, nb_old, init_cls=10, increment=10, task_boundaries=None):
    ...
    if task_boundaries is not None:
        # Use explicit per-task class boundaries: list of (start, end) exclusive pairs
        for (lo, hi) in task_boundaries:
            idxes = np.where(np.logical_and(y_true >= lo, y_true < hi))[0]
            label = "{}-{}".format(str(lo).rjust(2, "0"), str(hi - 1).rjust(2, "0"))
            all_acc[label] = (
                float("nan")
                if len(idxes) == 0
                else np.around(
