...
class IncrementalNet(BaseNet):
    ...
    def save_fc(self, filepath, task_id):
        os.makedirs(filepath, exist_ok=True)
        weight_path = os.path.join(filepath, f"CLs_weight{task_id}.pt")
        bias_path = os.path.join(filepath, f"CLs_bias{task_id}.pt")
        state_path = os.path.join(filepath, f"fc_state_{task_id}.pt")

        weight = self.fc.weight.detach().cpu()
        torch.save(weight, weight_path)
        if self.fc.bias is not None:
            bias = self.fc.bias.detach().cpu()
            torch.save(bias, bias_path)
        # Store the full state dict for robust restoration
        cpu_state = {k: v.detach().cpu() for k, v in self.fc.state_dict().items()}
        torch.save(cpu_state, state_path)

    def load_fc(self, filepath, task_id):
        state_path = os.path.join(filepath, f"fc_state_{task_id}.pt")
        weight_path = os.path.join(filepath, f"CLs_weight{task_id}.pt")
        bias_path = os.path.join(filepath, f"CLs_bias{task_id}.pt")
        ...
        if os.path.exists(weight_path):
            ...
            self.fc.weight.data.copy_(weight.to(self.fc.weight.device))
        if self.fc.bias is not None and os.path.exists(bias_path):
