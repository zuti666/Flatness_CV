...
def compute_first_vs_current_feature_drift(
    ...
    if args.get("feature_cka_eval", False):
        ...
        if X0 is not None and X0.numel() > 0:
            ...
            if XT is None or XT.numel() == 0:
                ...
                loader_first = DataLoader(dataset_first, batch_size=64, shuffle=False, num_workers=0, pin_memory=False)
                anchor_max_batches = int(args.get("feature_cka_max_batches", 8))
                X0, y0, _ = extract_features_and_labels(
                    model._network,
                    loader_first,
    ...
    if args.get("feature_proto_eval", False):
        ...
        if proto0 is None or protoT is None:
            ...
            loader_first = DataLoader(dataset_first, batch_size=64, shuffle=False, num_workers=0, pin_memory=False)
            features, labels, _ = extract_features_and_labels(
                model._network,
                loader_first,
                device=device_override,
                max_batches=int(args.get("feature_proto_max_batches", 8)),
            )
