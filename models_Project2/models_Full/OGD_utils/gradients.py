...
class GradientMemory:
    ...
    def add(self, v: torch.Tensor):
        ...
        if self.mode == 'orthonormal':
            ...
            for s in self.vectors:
                # Handle task-transition size changes (e.g., classifier expansion)
                # without reallocating all stored vectors.
                if v.numel() == s.numel():
                    proj = torch.dot(v, s) * s
                    v = v - proj
                else:
                    n = min(v.numel(), s.numel())
                    if n == 0:
                        continue
                    dot = torch.dot(v[:n], s[:n])
                    v[:n] = v[:n] - dot * s[:n]
    ...
    def project_orthogonal(self, g: torch.Tensor) -> torch.Tensor:
        ...
        g_tilde = g.clone()
        for s in self.vectors:
            if g_tilde.numel() == s.numel():
                s_use = s
                if s_use.device != g_tilde.device or s_use.dtype != g_tilde.dtype:
                    s_use = s_use.to(device=g_tilde.device, dtype=g_tilde.dtype)
                dot = torch.dot(g_tilde, s_use)
                g_tilde = g_tilde - dot * s_use
            else:
                n = min(g_tilde.numel(), s.numel())
                if n == 0:
                    continue
                s_part = s[:n]
                if s_part.device != g_tilde.device or s_part.dtype != g_tilde.dtype:
                    s_part = s_part.to(device=g_tilde.device, dtype=g_tilde.dtype)
                dot = torch.dot(g_tilde[:n], s_part)
                g_tilde[:n] = g_tilde[:n] - dot * s_part
        return g_tilde
