"""OGD + geometry-controlled RWP with OGD-safe correction.

Core update implemented per mini-batch:
1) clean gradient g at theta,
2) OGD drift projection q = P_S_perp g,
3) geometry-controlled perturbation delta and optional OGD noise projection,
...
with either isotropic Gaussian noise or old-task Fisher geometry.
"""
...
class Learner(BaseLearner):
    ...
    def _train(self, train_loader, test_loader):
        ...
        for epoch in prog_bar:
            ...
            for batch in train_loader:
                ...
                perturbations = self._sample_and_apply_rwp(trainable_params)
                if perturbations:
                    try:
                        optimizer.zero_grad()
    ...
    def _fisher_inv_sqrt_transform(
        ...
        if shuffle_eigvals:
            # Ablation: use same Fisher eigenvectors but random eigenvalue permutation.
            # Tests whether curvature-aligned weighting (1/λ) matters vs just the subspace.
    ...
    def _update_fisher_lowrank(self, loader, model: nn.Module) -> None:
        # Compute Fisher whenever fisher_rank > 0, regardless of noise_geometry.
        # "none" mode never needs Fisher; "gaussian"/"fisher" both benefit from it
        # (gaussian: for mechanism evaluation tr(HΣ) comparison).
        if self._noise_geometry == "none":
            logger.info("[OGD_FISHER3] Fisher update disabled for noise_geometry=%s", self._noise_geometry)
            return
        if self._fisher_rank <= 0:
            logger.info("[OGD_FISHER3] Fisher update disabled because fisher_rank=%d", self._fisher_rank)
            return
