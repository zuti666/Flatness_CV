...
class Learner(SeqLoRALearner):
    """SeqLoRA + GEM in delta_W space instead of raw A/B parameter space."""
    ...
    def _project_delta_grad(self, flat_delta_grad: torch.Tensor) -> torch.Tensor:
        if self._cur_task <= 0 or not self._task_grads or flat_delta_grad.numel() == 0:
            return flat_delta_grad
        memories = torch.stack(
            [
                _align_vector_dim(g, flat_delta_grad.numel(), flat_delta_grad.device, flat_delta_grad.dtype)
                for g in self._task_grads
            ],
            dim=0,
        )
        dotprod = memories @ flat_delta_grad
        if (dotprod < 0).any():
            return _project_multi(flat_delta_grad, memories, iters=self._gem_pgditers, lr=self._gem_pgdlr)
        return flat_delta_grad
    ...
    def incremental_train(self, data_manager):
        ...
        if g_task is not None:
            ...
            if self._is_main_process:
                self._log(f"[SeqLoRA_GEM_new] Stored task gradient {len(self._task_grads)}")
    ...
    def _compute_task_gradient(self, loader, model: nn.Module):
        ...
        try:
            for batch_idx, batch in enumerate(loader):
                ...
                loss.backward()
                g = self._flat_delta_grad(model).detach().cpu()
