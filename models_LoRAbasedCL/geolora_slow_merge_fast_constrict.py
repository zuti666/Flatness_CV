"""GeoLoRA Fast-Constrict + Slow Risk-Aware Merge.

On top of geolora_slow_merge_riskaware:
    - Fast layer (private atoms) is *constricted*: while fitting the new task, it is
      nudged away from old-task sensitive directions using first/second-order penalties
      in ΔW-space. Shared atoms stay frozen; their coefficients are *not* regularized.
    - Slow layer keeps the original risk-aware merge (utility - λ*risk) and Fisher
      subspace estimation for future tasks.

Second-order (SO) penalty:
    λ_so/2 · ⟨F_old_diag, (ΔW_priv)^2⟩   using KFAC-diag approx:
        cost_q = σ^2 · (b^2 · diag_out_q) · (a^2 · diag_in)
        cost_v analogous

First-order (FO) penalty (projection energy):
    λ_fo · occ_in(a) · occ_out(b) summed over private atoms, where occupancies use the
    old-task Fisher principal subspaces (same top-k as slow-merge risk).

No Fisher/EWC on shared coefficients c_t; only private basis {A,B,σ} is constrained.
"""
...
class GeoLoRA_QKV(nn.Module):
    ...
    def get_private_atoms(self):
        """
        Return orthonormalized private atoms only: {"q": (A,B,σ), "v": (...)}.
        σ is abs() so sign does not affect penalties.
        """
        def _pack(A, B, s):
            if A is None or B is None or s is None:
                return None
            Aq = self._orthonormal(A)
            Bq = self._orthonormal(B)
            sigma = s.abs()
            return Aq, Bq, sigma

        Aq, Bq, sq = _pack(self.theta_a_q_pr, self.theta_b_q_pr, self.sigma_q_pr)
        Av, Bv, sv = _pack(self.theta_a_v_pr, self.theta_b_v_pr, self.sigma_v_pr)
        return {"q": (Aq, Bq, sq), "v": (Av, Bv, sv)}
...
class Learner(LoraBaseLearner):
    def __init__(self, args):
        ...
        self._risk_diag_lambda = float(args.get("geolora_risk_diag_lambda", 0.0))

        # fast-layer constriction hyperparameters
        self._lambda_so = float(args.get("geolora_fast_lambda_so", 0.0))
        self._lambda_fo = float(args.get("geolora_fast_lambda_fo", 0.0))
        self._lambda_ortho = float(args.get("geolora_fast_lambda_ortho", 0.0))
        ...
        # fisher diagonals (KFAC diag approximation) for fast SO penalty
        self._fisher_diag_in: List[torch.Tensor | None] = []
        self._fisher_diag_out_q: List[torch.Tensor | None] = []
        self._fisher_diag_out_v: List[torch.Tensor | None] = []
    ...
    def _train(self, train_loader, test_loader):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for _, (_, inputs, targets) in enumerate(train_loader):
                ...
                # --- fast-layer constriction penalties (private atoms only) ---
                adapters = self._unwrap_network().backbone.adapters()
                if self._lambda_so > 0:
                    loss = loss + 0.5 * self._lambda_so * self._fast_so_penalty(adapters)
                if self._lambda_fo > 0:
                    loss = loss + self._lambda_fo * self._fast_fo_penalty(adapters)
                if self._lambda_ortho > 0:
                    loss = loss + self._lambda_ortho * self._fast_ortho_penalty(adapters)
    ...
    def _update_fisher_subspace(self, dataloader):
        ...
        self._ensure_fisher_slots(len(adapters))
        for qkv in adapters:
        ...
        for lidx, qkv in enumerate(adapters):
            ...
            # cache diagonals for fast SO penalty (diag of Kronecker ≈ diag_out ⊗ diag_in)
            self._fisher_diag_in[lidx] = torch.diag(self._fisher_cov_in[lidx]).detach().cpu()
            self._fisher_diag_out_q[lidx] = torch.diag(self._fisher_cov_out_q[lidx]).detach().cpu()
            self._fisher_diag_out_v[lidx] = torch.diag(self._fisher_cov_out_v[lidx]).detach().cpu()
        ...
        for lidx in range(len(adapters)):
            for branch in ["q", "v"]:
                ...
                for a, b in unique:
                    ...
                    risk_diag = self._risk_diag(lidx, branch, a, b) if self._risk_diag_lambda > 0 else 0.0
                    score = utility - self._risk_lambda * risk - self._risk_diag_lambda * risk_diag
                    scored.append((score, utility, risk, risk_diag, a, b))

                scored.sort(key=lambda x: x[0], reverse=True)

                selected: List[Tuple[torch.Tensor, torch.Tensor, float]] = []
                for score, utility, risk, risk_diag, a, b in scored:
    ...
    # ---------------- fast-layer constriction helpers ---------------- #
    def _ensure_fisher_slots(self, n: int):
        """Guarantee fisher lists have length n (fill with None)."""
        def _pad(lst):
            while len(lst) < n:
                lst.append(None)
        for lst in [
            self._fisher_cov_in,
            self._fisher_cov_out_q,
            self._fisher_cov_out_v,
            self._fisher_subspace,
            self._fisher_diag_in,
            self._fisher_diag_out_q,
            self._fisher_diag_out_v,
        ]:
            _pad(lst)

    def _fast_so_penalty(self, adapters: List[GeoLoRA_QKV]) -> torch.Tensor:
        """
        Diagonal Fisher penalty on *private* atoms only.
        Uses KFAC-diag approximation: diag(F_q) = diag_out_q ⊗ diag_in.
        """
        if self._lambda_so <= 0:
            return torch.tensor(0.0, device=self._device)

        pen = torch.zeros((), device=self._device)
        for lidx, qkv in enumerate(adapters):
            priv = qkv.get_private_atoms()
            if priv is None:
                continue
            diag_in = None if lidx >= len(self._fisher_diag_in) else self._fisher_diag_in[lidx]
            diag_out_q = None if lidx >= len(self._fisher_diag_out_q) else self._fisher_diag_out_q[lidx]
            diag_out_v = None if lidx >= len(self._fisher_diag_out_v) else self._fisher_diag_out_v[lidx]
            if diag_in is None or diag_out_q is None or diag_out_v is None:
                continue
            diag_in = diag_in.to(self._device)
            diag_out_q = diag_out_q.to(self._device)
            diag_out_v = diag_out_v.to(self._device)

            for branch, diag_out in (("q", diag_out_q), ("v", diag_out_v)):
                atoms = priv.get(branch)
                if atoms is None:
                    continue
                A, B, sigma = atoms
                ...
                b2 = B.pow(2)
                for i in range(A.shape[1]):
                    pen = pen + (sigma[i] ** 2) * torch.dot(b2[:, i], diag_out) * torch.dot(a2[:, i], diag_in)
        return pen

    def _fast_fo_penalty(self, adapters: List[GeoLoRA_QKV]) -> torch.Tensor:
        """
        Projection-energy FO penalty: penalize private atoms that lie in old-task
        Fisher principal subspaces (uses the same eigenspaces computed for risk).
        """
        if self._lambda_fo <= 0:
            return torch.tensor(0.0, device=self._device)
        ...
        for lidx, qkv in enumerate(adapters):
            priv = qkv.get_private_atoms()
            sub = self._fisher_subspace[lidx] if lidx < len(self._fisher_subspace) else None
            if priv is None or sub is None:
                continue
            Uin, Sin, tr_in = sub.get("U_in"), sub.get("S_in"), float(sub.get("tr_in", 0.0))
            if Uin is None or Sin is None or tr_in <= 0:
                continue
            branch_info = {
                "q": (sub.get("U_out_q"), sub.get("S_out_q"), float(sub.get("tr_out_q", 0.0))),
                "v": (sub.get("U_out_v"), sub.get("S_out_v"), float(sub.get("tr_out_v", 0.0))),
            }
            for branch, (Uout, Sout, tr_out) in branch_info.items():
                atoms = priv.get(branch)
                if atoms is None or Uout is None or Sout is None or tr_out <= 0:
                    continue
                A, B, sigma = atoms
                if A is None or B is None or sigma is None:
                    continue
                Ain = (Uin.t() @ A) ** 2  # [k_in, r]
                occ_in = (Ain.t() * Sin.view(-1)).sum(dim=1) / (tr_in + 1e-12)  # [r]
                Bout = (Uout.t() @ B) ** 2
                occ_out = (Bout.t() * Sout.view(-1)).sum(dim=1) / (tr_out + 1e-12)
                pen = pen + (sigma.pow(2) * occ_in * occ_out).sum()
        return pen

    def _fast_ortho_penalty(self, adapters: List[GeoLoRA_QKV]) -> torch.Tensor:
        """Orthogonality between shared and private bases (both A and B)."""
        if self._lambda_ortho <= 0:
            return torch.tensor(0.0, device=self._device)
        pen = torch.zeros((), device=self._device)
        for qkv in adapters:
            def _block(A_sh, B_sh, A_pr, B_pr):
                val = torch.zeros((), device=self._device)
                if A_sh is not None and A_pr is not None and A_sh.numel() > 0 and A_pr.numel() > 0:
                    A_sh_o = GeoLoRA_QKV._orthonormal(A_sh)
                    A_pr_o = GeoLoRA_QKV._orthonormal(A_pr)
                    val = val + (A_sh_o.t() @ A_pr_o).pow(2).sum()
                if B_sh is not None and B_pr is not None and B_sh.numel() > 0 and B_pr.numel() > 0:
                    B_sh_o = GeoLoRA_QKV._orthonormal(B_sh)
                    B_pr_o = GeoLoRA_QKV._orthonormal(B_pr)
                    val = val + (B_sh_o.t() @ B_pr_o).pow(2).sum()
                return val

            pen = pen + _block(qkv.theta_a_q_sh, qkv.theta_b_q_sh, qkv.theta_a_q_pr, qkv.theta_b_q_pr)
            pen = pen + _block(qkv.theta_a_v_sh, qkv.theta_b_v_sh, qkv.theta_a_v_pr, qkv.theta_b_v_pr)
        return pen

    def _risk_diag(self, lidx: int, branch: str, a: torch.Tensor, b: torch.Tensor) -> float:
        """
        Coordinate-weighted Fisher risk using stored diagonals (can originate from
        Yaoyue-style ΔW Fisher). risk = (b^2 · diag_out) * (a^2 · diag_in).
        """
        if lidx >= len(self._fisher_diag_in):
            return 0.0
        diag_in = self._fisher_diag_in[lidx]
        diag_out = self._fisher_diag_out_q[lidx] if branch == "q" else self._fisher_diag_out_v[lidx]
        if diag_in is None or diag_out is None:
            return 0.0
        return float(torch.dot(b.pow(2), diag_out.to(b.device)) * torch.dot(a.pow(2), diag_in.to(a.device)))
