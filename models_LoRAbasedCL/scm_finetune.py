...
class SlowConsolidationModule:
    ...
    def _greedy_select(
        ...
        for u, v, _ in candidates:
            ...
            scores.append((util, risk, util - lambda_risk * risk))
        ...
        with torch.no_grad():
            for name, param in self._network.named_parameters():
                if name in shared and name in self._pretrained_weights:
                    init_w = (self._pretrained_weights[name]
                              + shared[name]).to(param.device)
                    param.copy_(init_w)
        logger.info("[SCM] Applied shared init to %d layers.", len(shared))
