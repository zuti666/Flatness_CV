...
from evaluation_performance.probe import fit_linear_probe_softmax_head, evaluate_linear_probe_softmax_with_head
...
from eval_flat.param_utils import _select_params_by_name
...
def _train(args: Dict[str, Any]):
    ...
    # 0) I/O & logging
    # ---------------------------
    # ——— root: outputs_root/logs_all/<model>/<dataset> ———
    outputs_root = args.get("outputs_root", "outputs_logs")
    logs_root = os.path.join(outputs_root, "logs_all", str(args["model_name"]),str(args["optimizer_type"]), str(args["dataset"]),str(args["seed"]),str(args["prefix"]))
