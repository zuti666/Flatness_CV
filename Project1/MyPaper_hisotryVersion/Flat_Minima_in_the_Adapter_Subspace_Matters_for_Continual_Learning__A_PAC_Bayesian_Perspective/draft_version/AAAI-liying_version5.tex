...
\subsection{PAC-Bayesian Risk Bounds}
...
\section{PAC-Bayes Generalization Bound for Incremental LoRA with Flatness}
...
\subsubsection{Orthogonal incremental LoRA}
...
\paragraph{(1)Parameter Decomposition: Focusing KL on Adaptation Components}
...
\paragraph{(3)Flatness in Increnmatal LoRA}
...
\subsubsection{Reuse incremental LoRA}
Then we can consider another situation where the LoRA parameters of the new task are initialized from the LoRA of the previous task, and then continue training/fine-tuning.
