...
\section{PAC-Bayes Generalization Bound for Incre-
mental LoRA with Flatness}
...
\end{equation}
where $\mathcal{T}$ denotes the distribution over tasks, and $\mathcal{D}_t$ denotes the data distribution for task $t$. Here, $Q_t(\mathcal{D}_t, P)$ is the task-specific posterior induced by updating the prior $P$ with the data of task $t$. This quantity reflects the true generalization ability of the system but is \emph{intractable} in practice, as both the task distribution $\mathcal{T}$ and the underlying data distributions $\mathcal{D}_t$ are inaccessible. 
...
\end{aligned}
\end{equation}
where $S_t = {(x_{tj}, y_{tj})}_{j=1}^{m_t}$ denotes the training dataset for task $t$, and $\ell(\cdot,\cdot)$ is the task loss function. This empirical risk is the quantity actually observed during training, and serves as the basis for our generalization analysis.The three different risk are show in Fig.
...
% However, in practice, $L(\mathcal{Q})$ is not directly computable, as both the task distribution $\mathcal{T}$ and the data distributions $\mathcal{D}_t$ are unknown and inaccessible. We can only get the trained model based on  $n$ tasks, and get the genrializaion task on all $n$ task as follow: 

% $L(\mathcal{Q}_{1:n}) :=
%     \frac{1}{n} \sum_{i=1}^n \;
%       \mathbb{E}_{P_{1:n} \sim \mathcal{Q}_{1:n}} \;
%       \mathbb{E}_{f \sim Q_i(P_i)} 
%       \big[
%         L_{\mathcal{D}_i}(f)
%       \big] $


% To address this, we approximate the generalization risk by its empirical counterpart, utilizing the observed training set comprising $n$ tasks, leading to \textbf{the continual empirical risk}:
% \begin{equation}
% \begin{aligned}
%     \hat{L}(\mathcal{Q}) 
...
Recent advances in PAC-Bayesian theory for continual learning, such as the work of Pentina et al. (2014), provide valuable generalization guarantees but do not explicitly account for the flatness of the loss landscape, which has been empirically shown to correlate strongly with robustness and resistance to forgetting. To address this gap, we propose a novel PAC-Bayes generalization bound tailored for large-scale models employing IncLoRA, where each new task introduces an orthogonal and dedicated low-rank adaptation block. Our analysis explicitly integrates task-aware expected sharpness, a direct measure of flatness, into the generalization bound and employs a chain-dependent hyperdistribution to model recursive dependencies between tasks.
...
% \begin{theorem}[PAC-Bayes Generalization Bound for Incremental LoRA with Flatness]
% Consider $n$ sequential tasks under the Incremental LoRA framework, where for each task, the learned LoRA parameters are orthogonal to those of previous tasks, and the parameter posterior is Gaussian with chain-dependent hyperpriors. With probability at least $1-\delta$, the average generalization risk is bounded by:
% \begin{align}
%         L(\mathcal{Q}_{1:n}) \leq\; 
%         &  \hat{L}(\mathcal{Q}_{1:n}) + \bar{\mathrm{Sharpness}}(\mathcal{Q}_{1:n}) \notag \\
%         & + (b-a)\sqrt{
%             \frac{KL_{n}^{\text{task}} + KL_{n}^{\text{hyper}} \log(m/\delta)}{2(\bar{m}_{n} - 1)}
%         }
%     \end{align}
...
% \begin{align}
% &\bar{\mathrm{Sharpness}}(\mathcal{Q}_{1:n}) = 
%     \frac{1}{n} \sum_{t=1}^{n}
%         \mathbb{E}_{P_{1:n} \sim \mathcal{Q}_{1:n}}
...
%         \right] \\
% &\bar{m}_n = n \bigg/ \Big( \sum_{t=1}^n \frac{1}{m_t} \Big) \notag
% \end{align}
% \end{theorem}
...
\section{Flat-Minima Orthogonal Continual Learning:FMOCL}
...
Fig.\ref{fig3 result: epoch VS Learning rate} and Fig.\ref{fig4 result: epoch VS sample sizes} jointly illustrate the trade-offs among the learning rate, sample size, and training epochs across the three methods: SeqFT, IncLoRA, and FTLoRA (LoRA adaptation based on a finetuned model).
The results show that higher learning rates  or excessive training epochs lead to increased forgetting and reduced generalization, whereas a larger sample size contributes to improved overall model performance.
...
% Specifically, an excessively high learning rate and an excessive number of training epochs leads to increased forgetting and diminished generalization, while a larger sample size consistently enhances overall model performance. 
% LoRA method can achieve similar performance as Finetune. 
% The baseline configuration adopted in our experiments reflects a careful balance between optimizing performance on the current task and preserving the knowledge and generalizaiton.

\begin{figure}[h]
  \centering
  \includegraphics[width=1\linewidth]{Order3_accuracy_lr_epoch_compare-3methods.pdf}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=1\linewidth]{Order3_accuracy_samples_epoch_compare-2E10.pdf}
...
\subsubsection{Flatness between Finetune and LoRA}
