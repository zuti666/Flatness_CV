...
[4] Flat-LoRA: Low-Rank Adapation over a Flat Loss Landscape, ICML 2025.
...
The non-incremental contribution of the paper is theoretical rather than empirical observation that SAM can help LoRA. The novelty is not the generic observation that flatness can help, which is already known in full-model training and LoRA fine-tuning, but identifying which notion of sharpness remains theoretically relevant in frozen-backbone PECL once future adaptation is restricted to the admissible low-rank subspace. 

This is precisely the role of Theorem 4.4: under frozen-backbone PECL, both the bound-relevant sharpness term and the KL complexity reduce to quantities supported on $W_{\Delta,t}$. The added experiments are therefore used only to test the qualitative consequence of this reduction across broader model families, not to define the contribution itself.



Table 1. Average last-task accuracy across three task orders on Llama-3.2-1B/3B.
|Model|Method|Adam avg|AS(0) avg|Gain|
|-|-|-|-|-|
|Llama-3.2-1B|SeqLoRA|63.6|68.5|+4.9|
||IncLoRA|62.1|67.7|+5.6|
||OLoRA|56.7|63.6|+6.9|
|Llama-3.2-3B|SeqLoRA|67.5|73.2|+5.7|
||IncLoRA|67.2|72.8|+5.6|
||OLoRA|63.4|68.8|+5.4|
