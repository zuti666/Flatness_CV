...
from evaluation_sharpness.lanczos_iter import _lanczos_lambda_max_generic, _lanczos_topk_generic
from evaluation_sharpness.loss_utils import _compute_loss, _unwrap_batch
from evaluation_sharpness.param_utils import (
    _clone_params,
    _restore_params,
    _unflatten_to_param_like,
)
from evaluation_sharpness.power_iter import _power_iteration_lambda_max
...
def _loss_landscape_1d(
    ...
    """Sweep loss along a single random (or provided) direction in parameter space.

    Normalizes concatenated direction to unit norm and returns {x, loss} arrays."""
...
def _loss_landscape_2d(
    ...
    """Sweep loss over a 2D plane spanned by two directions (orthonormalized)."""
    backup = _clone_params(params)
...
def compute_full_vs_lora_curvature_1d(
    ...
    *,
    ...
    """Compare curvature-aligned 1D loss slices for full-parameter vs. LoRA subspaces."""
