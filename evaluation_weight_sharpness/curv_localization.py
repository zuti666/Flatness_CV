...
import torch
from evaluation_weight_sharpness.lanczos_iter import _lanczos_topk_generic, _lanczos_topk_masked
from evaluation_weight_sharpness.power_iter import _power_iteration_generic, _power_iteration_masked
from evaluation_weight_sharpness.loss_utils import _forward_logits_full, _unwrap_batch
...
def _curv_ts_eval(
    ...
    *,
    ...
    """Evaluate perturbation curvature term (Tuna/SAM-style) along adversarial direction."""
...
def _curv_noise_cov_eval(
    ...
    *,
    ...
    """Measure alignment between curvature eigenvectors and noise covariance (top-k)."""
...
def _delta_w_projection_eval(
    ...
    *,
    ...
    """Project delta-W (LoRA updates) onto curvature eigenvectors within a block."""
...
def _delta_w_full_projection_eval(
    ...
    *,
    ...
    """Project full effective weights (W) vs. delta-W onto curvature bases and save comps."""
...
def _w_delta_alignment_eval(
    ...
    *,
    ...
    """Quantify alignment between base weights and LoRA deltas (cosine/Frobenius)."""
...
def _mean_drift_eval(
    ...
    *,
    ...
    """Estimate mean-drift of LoRA (or similar) updates between tasks/steps.

    Computes ||Δ||^2, delta differences, and Gaussian log-mean terms for the
    configured block(s). Returns a flat metrics dict and optionally saves
    tensor payloads."""
...
def _lora_kl_eval(
    ...
    *,
    ...
    """Compute diagonal-Fisher KL between consecutive LoRA factors (task t vs t-1).

    Requires sequential LoRA mode; returns KL scalars and skips gracefully when
